...
const CreatePost = ({ onPostCreated }: CreatePostProps) => {
    ...
    const handlePost = async () => {
        ...
        try {
            const mediaUrl = selectedFile && previewUrl ? previewUrl : undefined;
            const response = await createPost({ content, mediaUrl });
            onPostCreated(response.post);
            closeModal();
        } catch (postError) {
            setError(postError instanceof Error ? postError.message : 'Unable to create post');
        } finally {
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file.size > 600 * 1024) {
            setError('Choose media under 600 KB until cloud storage is connected.');
            return;
        }
        ...
        reader.onerror = () => setError('Unable to read selected media.');
                ...
                <div style={styles.modalBackdrop}>
                    <div style={styles.modalContent}>
                        ...
                        <div style={styles.authorSection}>
                            <div style={styles.profileIcon}></div>
                            <span style={styles.modalAuthorName}>{user?.name || 'Student'}</span>
                        </div>
                        <textarea
                        ...
                        {previewUrl && (
                            <div style={styles.imagePreviewContainer}>
                                {mediaKind === 'video' ? (
                                    <video src={previewUrl} style={styles.imagePreview} controls />
                                ) : (
                        ...
                        <div style={styles.modalFooter}>
                            ...
                            <button
                                style={(postText.trim() || selectedFile) && !isSubmitting ? styles.postButton : styles.postButtonDisabled}
                                onClick={handlePost}
                                disabled={(!postText.trim() && !selectedFile) || isSubmitting}
                            >
                ...
                <div style={styles.modalBackdrop}>
                    <div style={styles.modalContent}>
                        ...
                        <div style={styles.goLiveBody}>
                            ...
                            <p style={styles.goLiveText}>Live streaming needs the media service in a later backend phase.</p>
                        </div>
                        <div style={styles.modalFooter}>
...
const styles: { [key: string]: React.CSSProperties } = {
    ...
    errorText: { color: '#DC2626', fontSize: '14px', margin: '0 20px 12px 20px' },
